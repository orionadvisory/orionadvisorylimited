...
function SidebarContent({
  ...
  return (
    <>
      {/* Logo */}
      <div className={cn("py-5 bg-[#17201c] border-b border-white/10", collapsed ? "px-3" : "px-5")}>
        <Link
        ...
        >
          ...
          {!collapsed && (
            <span className="text-base font-semibold text-[#fffaf1] tracking-tight">
              Orion{isAdmin ? " Admin" : ""}
            </span>
          )}
        </Link>
      ...
      {!isAdmin && !collapsed && (
        <div className="px-3 py-3 border-b border-white/10">
          <Link
            ...
            className="flex items-center gap-2.5 px-2 py-2 rounded-lg bg-white/[0.06] hover:bg-white/[0.1] transition-colors"
            onClick={onNavigate}
          >
            <div className="w-6 h-6 bg-emerald-300/15 rounded flex items-center justify-center flex-shrink-0">
              <Building2 className="w-3.5 h-3.5 text-emerald-200" />
            </div>
            <div className="min-w-0">
              <p className="text-xs font-medium text-[#fffaf1] truncate">
      ...
      {!isAdmin && collapsed && (
        <div className="px-3 py-3 border-b border-white/10 flex justify-center">
          <Link
            href="/dashboard/settings"
            className="w-8 h-8 bg-white/[0.08] rounded-lg flex items-center justify-center hover:bg-white/[0.12] transition-colors"
            onClick={onNavigate}
          ...
          return (
            <Link
              ...
              className={cn(
                ...
                isActive
                  ? "bg-[#fff5df] text-[#17201c] shadow-sm"
      ...
      <div className={cn("py-3 border-t border-white/10", collapsed ? "px-2" : "px-3")}>
        <Link
          href="/dashboard/settings"
          className={cn(
            "flex items-center rounded-lg hover:bg-white/[0.08] transition-colors",
            collapsed ? "justify-center px-2 py-2" : "gap-3 px-2 py-2"
          )}
          onClick={onNavigate}
          title={collapsed ? user.name : undefined}
        >
          <div className="w-7 h-7 rounded-full bg-[#d7a84f] flex items-center justify-center flex-shrink-0">
            <span className="text-[#17201c] text-xs font-semibold">
...
export function Sidebar({ user, startup }: SidebarProps) {
  ...
  return (
    <>
      ...
      <aside
        className={cn(
          "hidden md:flex fixed left-0 top-0 h-full bg-[#17201c] border-r border-black/10 flex-col z-30 transition-[width] duration-200 ease-out shadow-2xl shadow-black/10",
          collapsed ? "w-16" : "w-60"
        )}
      >
        <SidebarContent user={user} startup={startup} collapsed={collapsed} />
        {/* Collapse toggle */}
        <div className={cn("py-2 border-t border-white/10", collapsed ? "px-2" : "px-3")}>
          <button
            onClick={() => setCollapsed(!collapsed)}
            className={cn(
              "flex items-center rounded-lg text-stone-400 hover:bg-white/[0.08] hover:text-white transition-colors text-sm",
      ...
      <aside
        className={cn(
          "md:hidden fixed left-0 top-0 h-full w-72 bg-[#17201c] border-r border-black/10 flex flex-col z-50 transition-transform duration-200 ease-out shadow-2xl",
      ...
      >
        ...
        <div className="absolute top-4 right-4 z-10">
          <button
            onClick={() => setOpen(false)}
            className="w-8 h-8 rounded-lg flex items-center justify-center text-stone-300 hover:bg-white/[0.08] hover:text-white transition-colors"
          >
            <X className="w-5 h-5" />
          </button>
        </div>
